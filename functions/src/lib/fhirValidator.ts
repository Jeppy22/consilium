import { z } from 'zod';
import { FhirBundleSchema, type FhirBundle } from './schemas';

export type ValidationResult =
  | { ok: true; bundle: FhirBundle }
  | { ok: false; errors: string[] };

export function validateFhirBundle(input: unknown): ValidationResult {
  const parsed = FhirBundleSchema.safeParse(input);
  if (!parsed.success) {
...
function checkSemantics(bundle: FhirBundle): string[] {
  const errors: string[] = [];

  const patients = bundle.entry
    .map((e) => e.resource)
    .filter((r): r is Extract<typeof r, { resourceType: 'Patient' }> => r.resourceType === 'Patient');
  ...
  for (const entry of bundle.entry) {
    ...
    if (!match) {
      errors.push(
        `${r.resourceType}/${r.id}: subject.reference must be "Patient/<id>", got "${ref}".`
      );
      continue;
    }
    if (!patientIds.has(match[1])) {
      errors.push(
        `${r.resourceType}/${r.id}: subject.reference "Patient/${match[1]}" does not match any Patient in the bundle.`
      );
    }
  }
